...
4. Creates DataSets from the CSV files using the class *Dataset<Row>* and applies the schemas from the previous step.

5. Registers the DataSets from the previous step as temporary tables.

6. For each task given, executes a Spark SQL query and saves the resulting tables in a DataSet.

7. Prints the schemas for the DataSets created in the previous step (for tesing purposes).

8. Exports the DataSets in CSV format and saves them on disc for further analysis.
